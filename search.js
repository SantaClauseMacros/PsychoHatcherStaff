...
// Simple search functionality for the Knowledge Base
document.addEventListener('DOMContentLoaded', function() {
  const searchInput = document.querySelector('[id^="kb-search-input"]');
  const searchButton = document.querySelector('[id^="kb-search-button"]');
  ...
  function performSearch() {
    ...
    // Show search results
    const resultsContainer = document.querySelector('.kb-search-results');
    if (resultsContainer) {
      resultsContainer.innerHTML = '';
      resultsContainer.appendChild(searchResults);
      resultsContainer.classList.add('active');
      
      // Simulate search (in a real application, this would query a database)
      setTimeout(() => {
        const guides = [
          { title: "Rankup Macro Guide", url: "guide-rankup.html" },
          { title: "Digging Mode Guide", url: "guide-digging.html" },
          { title: "Clan Mode Guide", url: "guide-clan.html" },
          { title: "Treehouse Mode Guide", url: "guide-treehouse.html" },
          { title: "Fishing Mode Guide", url: "guide-fishing.html" },
          { title: "Common Issues Guide", url: "guide-common.html" }
        ];
        ...
        if (matchingGuides.length > 0) {
          searchResults.innerHTML = `<h4>Search Results</h4><ul class="search-results-list"></ul>`;
          const resultsList = searchResults.querySelector('.search-results-list');
          
          matchingGuides.forEach(guide => {
            const resultItem = document.createElement('li');
            resultItem.innerHTML = `<a href="${guide.url}">${guide.title}</a>`;
...
function showNotification(message, type) {
  ...
  // Create a notification if the main function doesn't exist
